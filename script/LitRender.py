...
def fcid(input_id) -> tuple:
    ...
    """
    color_data=json.load(open('../lang/zh_cn.json','r',encoding="UTF-8"))
...
def main_render_loop(blocks, rotate:bool):
    global xl, yl, zl
    xl = max([x for (x, _, _), _ in blocks])
    yl = max([y for (_, y, _), _ in blocks])
    zl = max([z for (_, _, z), _ in blocks])
    print(f"{(xl, yl, zl)}")
    pos_blocks = [[[None] * (zl + 1) for _ in range(yl + 1)] for _ in range(xl + 1)]
    for position, block_id in blocks:
        x, y, z = position
        pos_blocks[x][y][z] = block_id
    print(pos_blocks)
...
class Minecraft3DApp:
    def __init__(self, root):
        self.root = root
        self.root.title("我的世界3D渲染")
        self.frame = ttk.Frame(self.root)
        self.frame.pack(fill="both", expand=True)
        self.label = ttk.Label(self.frame, text="3D渲染窗口")
        self.label.pack()
        self.button = ttk.Button(self.frame, text="启动3D渲染", command=self.start_render)
        self.button.pack()
    def start_render(self):
        # 启动3D渲染
        render_thread = threading.Thread(target=main_render_loop, args=(blocks,True))
        render_thread.start()
